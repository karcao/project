...
Page({
    ...
    huanyipi:function(){
        ...
        let gradeid = "";
        console.log(that.data.cdposition.working_life)
        if (that.data.cdposition.working_life < 1) {
            gradeid = 1
        } if (that.data.cdposition.working_life >= 1 && that.data.cdposition.working_life < 3) {
            gradeid = 2
        } if (that.data.cdposition.working_life >= 3 && that.data.cdposition.working_life < 5) {
            gradeid = 3
        } if (that.data.cdposition.working_life >= 5 && that.data.cdposition.working_life < 10) {
            gradeid = 4
        } if (that.data.cdposition.working_life >= 10) {
            gradeid = 5
        }
        console.log(gradeid)
        wx.request({
            url: app.data.apiPath + '/webCompany/searchCustomerInfoByPositionName',
            method: "POST",
            header: {
            ...
            data: {
                ...
                professionid: that.data.cdposition.position_id,
                educationid: that.data.cdposition.education,
                unionid: that.data.cdposition.unionid,
                gradeid: gradeid,
                positionName: that.data.cdposition.position_name
            },
        ...
        if (options.positionId){
            ...
            wx.request({
                ...
                success: function (res) {
                    console.log(res)
                    console.log(JSON.parse(res.data).data)
                    let cdposition = JSON.parse(res.data).data;
                    that.setData({
                        cdposition
                    })
                    let gradeid = "";
                    if (cdposition.working_life < 1) {
                        gradeid = 1
                    } if (cdposition.working_life >= 1 && cdposition.working_life < 3) {
                        gradeid = 2
                    } if (cdposition.working_life >= 3 && cdposition.working_life < 5) {
                        gradeid = 3
                    } if (cdposition.working_life >= 5 && cdposition.working_life < 10) {
                        gradeid = 4
                    } if (cdposition.working_life >= 10) {
                        gradeid = 5
                    }
                    //判断工作年限
                    wx.request({
                        ...
                        data: {
                            pageSize: that.data.pageSize,
                            professionid: cdposition.position_id,
                            educationid: cdposition.education,
                            unionid: cdposition.unionid,
                            gradeid: gradeid,
                            positionName: cdposition.position_name
                        },
                        success: function (res) {
                            wx.hideLoading();
                            console.log(res)
            ...
            console.log("是从详情页面进入")
            console.log(JSON.parse(options.position))
            let cdposition = JSON.parse(options.position)
            that.setData({
                cdposition
            })
            let gradeid = "";
